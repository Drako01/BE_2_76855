...
export default class CustomRouter {
    ...
    get(path, ...handlers) { this.router.get(path, ...handlers.map(h => this._warp(h)));}
    post(path, ...handlers) { this.router.post(path, ...handlers.map(h => this._warp(h)));}
    put(path, ...handlers) { this.router.put(path, ...handlers.map(h => this._warp(h)));}
    delete(path, ...handlers) { this.router.delete(path, ...handlers.map(h => this._warp(h)));}
